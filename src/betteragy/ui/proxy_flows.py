"""UI panel and event handlers for the Zero-Restart Local Proxy screen."""
...
def render_proxy_panel(tui) -> Panel:
    """Render status and control panel for the local zero-restart proxy."""
    pid = get_proxy_pid()
    ...
    ca_file = DEFAULT_CERTS_DIR / "ca.crt"

    text = Text()
    text.append("Betteragy Transparent Local Proxy", style="bold cyan")
    text.append(" -- Zero-Restart Switching & 429 Auto-Rotation\n\n", style="bold white")
    ...
    if pid and healthy:
        ...
        text.append("agy Environment Configuration:\n", style="bold white")
        text.append(f"  export HTTPS_PROXY=http://{DEFAULT_PROXY_HOST}:{DEFAULT_PROXY_PORT}\n", style="cyan")
        text.append(f"  export SSL_CERT_FILE={ca_file}\n", style="cyan")
        text.append("  (Automatically injected when using 'agy' shell wrapper)\n\n", style="dim")
        border = "green"
    else:
        text.append("Status:         ", style="dim")
        text.append("[!] STOPPED\n\n", style="bold yellow")
        ...
        text.append("Switch accounts in Betteragy without restarting agy sessions.\n", style="white")
        text.append("  [*] 429 Auto-Rotation:     ", style="bold cyan")
        text.append("When current account hits rate-limits, proxy auto-swaps to next healthy account.\n\n", style="white")
    ...
    return Panel(Group(text, instructions), title="[~] Local Proxy Controller", border_style=border, box=DEFAULT_BOX)
