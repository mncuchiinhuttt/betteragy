...
def render_accounts_table(accounts: list[AccountRecord], active_email: Optional[str]) -> Table:
    ...
    table = Table(
        title="[bold cyan]Antigravity Multi-Account Switchboard[/bold cyan]",
        box=DEFAULT_BOX,
    ...
    table.add_column("Last Switched", style="dim", justify="right", width=16)

    now = time.time()
    for idx, acc in enumerate(accounts, 1):
        is_active = bool(active_email and acc.email.lower() == active_email.lower())
        is_cooldown = acc.cooldown_until > now
        badge = format_status_badge(is_active, is_cooldown, acc.disabled)
        ...
        row_style = "bold on #1a2332" if is_active else None
...
def render_quota_table(quota: AccountQuota) -> Table:
    ...
    title = f"[bold cyan]AI Model Quota — {quota.email}{tier_info}[/bold cyan]"
    table = Table(title=title, box=DEFAULT_BOX, header_style="bold magenta", title_justify="left")

    table.add_column("Model", style="bold white", min_width=28)
    table.add_column("Available Quota", min_width=24)
    table.add_column("Reset Time", style="cyan", justify="center", width=12)
    table.add_column("Countdown", style="yellow", justify="right", width=14)

    if quota.is_error:
        table.add_row("[red]Error[/red]", f"[red]{quota.error_message}[/red]", "-", "-")
        return table

    if quota.is_forbidden:
        table.add_row("[yellow]Forbidden[/yellow]", "No Code Assist quota granted", "-", "-")
        return table

    for b in quota.buckets:
        bar = render_progress_bar(b.percentage)
...
def render_multi_quota_matrix(quotas: list[AccountQuota]) -> Table:
    ...
    table = Table(
        title="[bold cyan]Live Quota Matrix Across All Accounts[/bold cyan]",
        box=DEFAULT_BOX,
        header_style="bold magenta",
        title_justify="left",
    )
    ...
    # Collect all model display names
    ...
    for model_id, display_name in sorted(all_models.items(), key=lambda x: x[1]):
        ...
        for q in quotas:
            ...
            if bucket:
                row.append(render_progress_bar(bucket.percentage, width=8))
            else:
                row.append("[dim]--[/dim]")
        table.add_row(*row)
...
def render_top_conversations_table(report: DeepUsageReport) -> Table:
    ...
    table = Table(
        title="[bold cyan]Top Conversations by Token & Cost Usage[/bold cyan]",
        box=DEFAULT_BOX,
        header_style="bold magenta",
        title_justify="left",
    )
    ...
    table.add_column("Input", justify="right", style="cyan", width=8)
    table.add_column("Cache", justify="right", style="green", width=8)
    table.add_column("Output", justify="right", style="magenta", width=8)
    table.add_column("Reason", justify="right", style="yellow", width=8)
    table.add_column("Total", justify="right", style="bold white", width=9)
    table.add_column("Cost", justify="right", style="bold green", width=9)
...
def render_model_breakdown_table(models: list) -> Table:
    ...
    table = Table(
        title="[bold cyan]Token Usage & Estimated Cost by Model[/bold cyan]",
        box=DEFAULT_BOX,
        header_style="bold magenta",
        title_justify="left",
    )
    table.add_column("Model Group", style="bold white", min_width=28)
    table.add_column("Input", justify="right", style="cyan", width=9)
    table.add_column("Cache", justify="right", style="green", width=9)
    table.add_column("Output", justify="right", style="magenta", width=9)
    table.add_column("Reason", justify="right", style="yellow", width=9)
    table.add_column("Total Tokens", justify="right", style="bold white", width=12)
    table.add_column("Est. Cost", justify="right", style="bold green", width=10)
