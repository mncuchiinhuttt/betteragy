...
class InteractiveTUI:
    ...
    def run(self) -> None:
        ...
        # Enter alternate terminal buffer & hide cursor
            ...
            with KeyListener() as listener:
                while True:
                    ...
                    if self.current_screen == "main":
                        ...
                        self._handle_switch_key(key)
                    elif self.current_screen in ("quota", "usage", "shell"):
                        if key in (KEY_ESC, KEY_BACK, KEY_QUIT, KEY_ENTER):
                            self.current_screen = "main"
                        elif key == KEY_REFRESH and self.current_screen == "quota":
                            self.cached_quota = None
        finally:
            # Leave alternate buffer & restore cursor
    ...
    def _render_current_view(self) -> None:
        ...
        if self.status_message:
            elements.append(
                Panel(self.status_message, style="bold green", box=DEFAULT_BOX)
            )
        ...
        if self.current_screen == "main":
            elements.append(render_main_menu_panel(self.menu_idx, active_email))
            elements.append(render_footer_hints("main"))
        elif self.current_screen == "switch_account":
            accounts = self.acc_svc.get_accounts()
            elements.append(
                render_account_selector_panel(accounts, self.account_idx, active_email)
            )
            ...
            elements.append(Panel(syntax, title="[bold cyan]🐚 Shell Integration[/bold cyan]", box=DEFAULT_BOX))
            elements.append(render_footer_hints("sub"))
    ...
    def _handle_main_key(self, key: str) -> bool:
        ...
        if key == KEY_UP:
            self.menu_idx = (self.menu_idx - 1) % total_items
            self.status_message = ""
        elif key == KEY_DOWN:
            self.menu_idx = (self.menu_idx + 1) % total_items
            self.status_message = ""
            ...
            self.status_message = ""
            selected_action = MAIN_MENU_ITEMS[self.menu_idx][0]

            if "Switch Account" in selected_action:
                self.current_screen = "switch_account"
                self.account_idx = 0
            elif "Live AI Quotas" in selected_action:
                self.cached_quota = None
                self.current_screen = "quota"
            elif "Token Usage" in selected_action:
                self.cached_report = None
                self.current_screen = "usage"
            elif "Rotate Account" in selected_action:
                ok, msg = self.rot_svc.rotate()
                self.status_message = f"[bold green]✓ {msg}[/bold green]" if ok else f"[red]✗ {msg}[/red]"
            elif "Set Cooldown" in selected_action:
                ok, msg = self.rot_svc.set_cooldown(hours=4.0)
                self.status_message = f"[yellow]{msg}[/yellow]"
            elif "Add Account" in selected_action:
                self.status_message = "To add an account, run: betteragy account add (or --token)"
            elif "Remove Account" in selected_action:
                self.status_message = "To remove an account, run: betteragy account remove <email>"
            elif "Shell Integration" in selected_action:
                self.current_screen = "shell"
            elif "Exit" in selected_action:
                return True
        return False

    def _handle_switch_key(self, key: str) -> None:
        """Handle key input on account selector screen."""
        accounts = self.acc_svc.get_accounts()
        if not accounts:
            self.current_screen = "main"
            return
        ...
        if key == KEY_UP:
            self.account_idx = (self.account_idx - 1) % len(accounts)
        elif key == KEY_DOWN:
            self.account_idx = (self.account_idx + 1) % len(accounts)
